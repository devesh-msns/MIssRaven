...
def blacklist(bot: Bot, update: Update, args: List[str]):
    ...
    for text in split_text:
        if text == base_blacklist_string:
            if update_chat_title == message_chat_title:
                msg.reply_text("There are no blacklisted messages here! u can add one tho")
            else:
                msg.reply_text(f"There are no blacklisted messages in <b>{update_chat_title}</b> u can add it tho!",
...
def add_blacklist(bot: Bot, update: Update):
    ...
    if len(words) > 1:
        ...
        if len(to_blacklist) == 1:
            msg.reply_text(f"Added <code>{html.escape(to_blacklist[0])}</code> to the blacklist!hehe",
                           parse_mode=ParseMode.HTML)
...
def unblacklist(bot: Bot, update: Update):
    ...
    if len(words) > 1:
        ...
        if len(to_unblacklist) == 1:
            ...
            msg.reply_text("None of these triggers exist, so they weren't removed.really?", parse_mode=ParseMode.HTML)
