...
def check_flood(bot: Bot, update: Update) -> str:
    ...
    try:
        ...
        msg.reply_text("I can't kick people here, give me permissions first! sadly, Until then, I'll disable antiflood, sorry.")
        sql.set_flood(chat.id, 0)
        log_message = ("<b>{chat.title}:</b>\n"
                       "#INFO\n"
                       "Don't have kick permissions,Pls give it , till then ill automatically disabled antiflood.")
...
def set_flood(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if len(args) >= 1:
        ...
        if val == "off" or val == "no" or val == "0":
            sql.set_flood(chat.id, 0)
            message.reply_text("As per your wish , i have disabled anti flood from {}.".format(chat_name), parse_mode=ParseMode.HTML)
            ...
            if amount <= 0:
                ...
                log_message = (f"<b>{html.escape(chat.title)}:</b>\n"
                               ...
                               f"As per your wish , i have disabled anti flood from")

                return log_message
            elif amount < 3:
                message.reply_text("Antiflood has to be either 0 (disabled), or a number bigger than 3! , u cant consider a flood below 3 ")
                return log_message
