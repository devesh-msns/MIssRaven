...
def ban(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not user_id:
        message.reply_text("I doubt that's a user, u sure? cous i m not.")
        return log_message
    ...
    try:
        ...
        if excp.message == "User not found":
            message.reply_text("Can't seem to find this person.where did u saw em?")
            return log_message
        else:
            raise

    if user_id == bot.id:
        message.reply_text("Dont ban me ! pleaseee ! i love u so muchh !!! why u hate me? tell at support grp")
        return log_message

    # dev users to bypass whitelist protection incase of abuse
    if is_user_ban_protected(chat, user_id, member) and user not in DEV_USERS:
        message.reply_text("This user is a god to me , i sadly wont ban him")
        return log_message
    ...
    try:
        ...
        bot.sendMessage(chat.id, "haha, u did bad things? Banned user {}.".format(mention_html(member.user.id, member.user.first_name)),
                        parse_mode=ParseMode.HTML)
        return log

    except BadRequest as excp:
        if excp.message == "Reply message not found sorry":
            # Do not reply
            message.reply_text('Boom! Banned!', quote=False)
            return log
        else:
            LOGGER.warning(update)
            LOGGER.exception("ERROR banning user %s in chat %s (%s) due to %s", user_id, chat.title, chat.id,
                             excp.message)
            message.reply_text("Uhm...that didn't work...sorry 
            9ll")
