...
def promote(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    if not chatD.get_member(bot.id).can_promote_members:
        update.effective_message.reply_text("Em sorry i cant do it ! "
                                            "Make sure I'm admin with the right to appoint new admins !.)
        exit(1)

    user_id = extract_user(message, args)
    if not user_id:
        message.reply_text(tld(chat.id, "Who are you promoting? Let me know my replying their message or tagging them ! "))
        return ""

    user_member = chatD.get_member(user_id)
    if user_member.status == 'administrator' or user_member.status == 'creator':
        message.reply_text(tld(chat.id, "Isnt he already an admin? Nice joke!"))
        return ""

    if user_id == bot.id:
        message.reply_text(tld(chat.id, "I cant promote myself !, can u do it for me?"))
        return ""
    ...
    message.reply_text(tld(chat.id, f" Yaya!, {mention_html(user_member.user.id, user_member.user.first_name)} just got admin rights in {html.escape(chatD.title)}!Enjoy!!!"), parse_mode=ParseMode.HTML)
...
def demote(bot: Bot, update: Update, args: List[str]) -> str:
    ...
    user_id = extract_user(message, args)
    if not user_id:
        message.reply_text(tld(chat.id, "Whom to demote? reply or tag them !!"))
        return ""
    ...
    if user_id == bot.id:
        message.reply_text(tld(chat.id, "I can't demote myself! pleaaseeee noo! how can i get better??? tell at my support grp !!))
        return ""
    ...
    try:
        ...
        message.reply_text(tld(chat.id, f"Successfully demoted in *{chatD.title}*!  Better luck next time boi "), parse_mode=ParseMode.MARKDOWN)
        return f"<b>{html.escape(chatD.title)}:</b>" \
                "\n#DEMOTED" \
               f"\n<b>Admin:</b> {mention_html(user.id, user.first_name)}" \
               f"\n<b>User:</b> {mention_html(user_member.user.id, user_member.user.first_name)}"
        ...
        message.reply_text(
            tld(chat.id, "I cant demote them! only person who promoted them or the creator can! telegram's rule .. cant  ")
            )
        return ""
...
def invite(bot: Bot, update: Update):
    ...
    if chat.username:
        ...
        update.effective_message.reply_text("I can only give you invite links for supergroups and channels, sorry! (convert to supergroup if not) ")
...
def set_title(bot: Bot, update: Update, args: List[str]):
    ...
    if not user_id:
        message.reply_text("You don't seem to be referring to a user. reply them or tag em")
        return

    if user_member.status == 'creator':
        message.reply_text("This person CREATED the chat, i cant do that")
        return

    if not user_member.status == 'administrator':
        message.reply_text("Can't set title for non-admins!\nPromote them man ! thn u can set custom title!")
        return
    ...
    if not title:
        message.reply_text("Setting blank title doesn't do anything! really dude?")
        return

    if len(title) > 16:
        message.reply_text("Boi , The title length is longer than 16 characters,.\nTruncating it to 16 characters.")
    ...
    if status is True:
        ...
        if description == "Bad Request: not enough rights to change custom title of the user.. sed":
            message.reply_text("I can't set custom title for admins that I didn't promote! oopsy")
...
def setchatpic(bot: Bot, update: Update):
    ...
    if user_member.can_change_info == False:
       msg.reply_text("You are missing right to change group info! if you cant do it , how can i?")
       return
    ...
    if msg.reply_to_message:
       if msg.reply_to_message.photo:
          ...
          return
       dlmsg = msg.reply_text("Hold on a sec...")
       tpic = bot.get_file(pic_id)
       tpic.download('gpic.png')
       try:
          with open('gpic.png', 'rb') as chatp:
               bot.set_chat_photo(int(chat.id), photo=chatp)
               msg.reply_text("Successfully set new chat Picture! Congo")
       except BadRequest as excp:
          msg.reply_text(f"shit, some Error! {excp.message}")
       finally:
          dlmsg.delete()
...
def rmchatpic(bot: Bot, update: Update):
    ...
    user_member = chat.get_member(user.id)
    if user_member.can_change_info == False:
       msg.reply_text("You don't have enough rights to delete group photo, sed")
       return
    try:
        bot.delete_chat_photo(int(chat.id))
        msg.reply_text("Successfully deleted chat's profile photo! boom!")
    except BadRequest as excp:
